...
class _ExportImportScreenState extends State<ExportImportScreen> {
  ...
  Future<void> _exportInventory() async {
    ...
    try {
      ...
      await Share.shareXFiles(
        ...
        text:
            'Inventory exported on ${DateFormat('dd MMM yyyy').format(DateTime.now())}',
      );
  ...
  Future<void> _exportTransactions() async {
    ...
    try {
      ...
      final timestamp = DateFormat('yyyyMMdd_HHmmss').format(DateTime.now());
      final file =
          File('${directory.path}/bharat_transactions_$timestamp.json');
      await file.writeAsString(jsonString);
      ...
      await Share.shareXFiles(
        ...
        text:
            'Transactions exported on ${DateFormat('dd MMM yyyy').format(DateTime.now())}',
      );
  ...
  Future<void> _importInventory() async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('Inventory imported successfully'),
              backgroundColor: Colors.green),
        );
      }
    } catch (e) {
  ...
  Future<void> _importTransactions() async {
    try {
      ...
      final bills =
          (data['bills'] as List).map((b) => Bill.fromJson(b)).toList();
  ...
  Future<String?> _showImportDialog(String type, int count) async {
    return showDialog<String>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
            'Found $count item${count != 1 ? 's' : ''}. How do you want to import?'),
        actions: [
          TextButton(
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const Text(
                            '• Exports create JSON files on your device'),
                        const Text(
                            '• Use "Share" to send files via WhatsApp, Email, etc.'),
                        const Text('• Import files from device storage'),
                        const Text('• Merge: Keep existing + add new items'),
                        const Text(
                            '• Replace: Delete all + add imported items'),
                        const SizedBox(height: 8),
                        Text(
