        ...
        builder: (context, billsBox, _) {
          return ValueListenableBuilder<Box<Product>>(
            ...
            builder: (context, productsBox, _) {
              ...
              // Filter Bills
              final periodBills = billsBox.values
                  .where((b) => b.timestamp.isAfter(startDate))
                  .toList();

              // Revenue
              final totalRevenue =
                  periodBills.fold<double>(0, (sum, b) => sum + b.total);
              ...
              final prevStartDate =
                  startDate.subtract(Duration(days: _selectedDays));
              final prevBills = billsBox.values
                  .where((b) =>
                      b.timestamp.isAfter(prevStartDate) &&
                      b.timestamp.isBefore(startDate))
                  .toList();
              final prevRevenue =
                  prevBills.fold<double>(0, (sum, b) => sum + b.total);
              final growth = prevRevenue > 0
              ...
              for (int i = 0; i < _selectedDays; i++) {
                ...
                dailyMap[DateTime(d.year, d.month, d.day)
                    .millisecondsSinceEpoch] = 0.0;
              }
              for (var b in periodBills) {
                final d = DateTime(
                    b.timestamp.year, b.timestamp.month, b.timestamp.day);
                dailyMap[d.millisecondsSinceEpoch] =
                    (dailyMap[d.millisecondsSinceEpoch] ?? 0) + b.total;
              }
              ...
              for (var b in periodBills) {
                for (var item in b.items) {
                  ...
                  productRevMap[item.productId] =
                      (productRevMap[item.productId] ?? 0) + item.total;
              ...
              for (var b in billsBox.values) {
                // Check ALL bills for last sale
                for (var item in b.items) {
              ...
              for (var b in periodBills) {
                for (var item in b.items) {
                  ...
                  final p = productsBox
                      .get(item.productId); // Might need null check if deleted
              ...
              final dayCounts =
                  <int, double>{}; // Weekday -> Revenue (or count)

              for (var b in billsBox.values) {
                // Analyse ALL time for better patterns, or periodBills? Let's use periodBills for trend
              ...
              final peakHour12 = peakHour > 12
                  ? peakHour - 12
                  : (peakHour == 0 ? 12 : peakHour);
              final peakHourTime = maxHourlyCount == 0
                  ? '--'
                  : '$peakHour12 $peakSuffix - ${peakHour12 == 12 ? 1 : peakHour12 + 1} $peakSuffix';
              ...
              final busiestDayName =
                  maxDayRev == 0 ? '--' : days[busiestDay - 1];
              ...
              final topCatPct = totalRevenue > 0
                  ? (topCatRev / totalRevenue * 100).toInt()
                  : 0;
              ...
              final totalBaskets =
                  periodBills.length > 0 ? periodBills.length : 1;

              // Smart Insights Generation (Moved here)
              final insights = <String>[];
              if (topProductsQty.isNotEmpty) {
                final topName =
                    productNameMap[topProductsQty.first.key] ?? 'Item';
                final val = topProductsQty.first.value;
                insights.add(
                    '$topName is trending — highest sales: ${val.toInt()} units.');
              }
              if (deadStock.isNotEmpty) {
                insights.add(
                    '${deadStock.length} products haven\'t sold in 30 days — consider clearing.');
              }
              // Category Contribution
              if (topCategoryName != 'None') {
                insights.add(
                    '$topCategoryName category contributes $topCatPct% to your revenue.');
              }
              if (insights.isEmpty)
                insights
                    .add('Collect more sales data to generate smart insights.');
              ...
              return SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(fullCurrencyFormat.format(totalRevenue),
                              style: const TextStyle(
                                  color: Colors.white,
                                  fontSize: 32,
                                  fontWeight: FontWeight.bold)),
                          ...
                          Row(
                            children: [
                              _buildWhiteMetric(
                                  'Bills', totalOrders.toString()),
                              const SizedBox(width: 24),
                              _buildWhiteMetric('Avg Order',
                              ...
                              children: [
                                Row(children: [
                                  Icon(Icons.calendar_today_rounded,
                                      color: AppTheme.electricPurple, size: 18),
                                  SizedBox(width: 8),
                                  Text('Busiest Day',
                                      style: TextStyle(
                                          color: AppTheme.textMuted,
                        ...
                        Expanded(
                          child: Container(
                            ...
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                const Text('Basket Size',
                                    style: TextStyle(
                                        fontWeight: FontWeight.bold,
                                        fontSize: 13)),
                                const SizedBox(height: 12),
                                _buildBasketBar('Small', basketSmall,
                                    totalBaskets, Colors.blue),
                                const SizedBox(height: 8),
                                _buildBasketBar('Med', basketMedium,
                                    totalBaskets, Colors.orange),
                                const SizedBox(height: 8),
                                _buildBasketBar('Large', basketLarge,
                                    totalBaskets, Colors.teal),
                              ],
                            ),
                          ),
                        ),
                            ...
                            children: [
                              ListTile(
                                ...
                                title: Text(name,
                                    style: const TextStyle(
                                        fontWeight: FontWeight.bold)),
                                trailing: Text('${item.value.toInt()} units',
                                    style: const TextStyle(
                                        fontWeight: FontWeight.w600,
                                        color: AppTheme.darkNavy)),
                              ),
                              if (index != topProductsQty.take(5).length - 1)
                                const Divider(
                              ...
                              ListTile(
                                ...
                                title: Text(name,
                                    style: const TextStyle(
                                        fontWeight: FontWeight.bold)),
                                trailing: Text(
                                    fullCurrencyFormat.format(item.value),
                                    style: const TextStyle(
                                        fontWeight: FontWeight.w600,
                                        color: AppTheme.darkNavy)),
                              ),
                              if (index != topProductsRev.take(5).length - 1)
                                const Divider(
                                    height: 1, indent: 16, endIndent: 16),
                            ],
                          );
                    ...
                    const Text('Dead & Slow Stock',
                        style: TextStyle(
                            fontSize: 18,
                            fontWeight: FontWeight.bold,
                            color: AppTheme.error)),
                    const SizedBox(height: 4),
                    Text('Items with no sales in 30+ days',
                        style:
                        ...
                        child: Column(
                          children: deadStock.take(5).map((p) {
                            ...
                            final days = last == null
                                ? 'Never Sold'
                                : '${now.difference(last).inDays}d ago';
                            return ListTile(
                              title: Text(p.name,
                                  style: const TextStyle(
                                      fontWeight: FontWeight.w600)),
                        ...
                        children: [
                          Row(children: [
                            Icon(Icons.auto_awesome,
                                color: AppTheme.darkNavy, size: 20),
                            const SizedBox(width: 8),
                      ...
                      child: OutlinedButton.icon(
                        onPressed: () {
                          ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                  content:
                                      Text('Export feature coming in v1.1')));
                        },
