...
class _DashboardScreenState extends State<DashboardScreen> {
          ...
          builder: (context, billsBox, _) {
            return ValueListenableBuilder<Box<Product>>(
                ...
                builder: (context, productsBox, _) {
                  ...
                  final todayRevenue =
                      todayBills.fold<double>(0, (sum, b) => sum + b.total);

                  // 2. Low Stock
                  final products = productsBox.values.toList();
                  final lowStockItems =
                      products.where((p) => p.quantity <= 5).toList();
                  final outOfStockItems = products
                      .where((p) => p.quantity == 0)
                      .toList(); // Subset of low stock usually, or strict 0
                  ...
                  for (var bill in todayBills) {
                    for (var item in bill.items) {
                      itemSales[item.name] =
                          (itemSales[item.name] ?? 0) + item.quantity;
                    }
                  }
                  ...
                  final yesterdayRevenue =
                      yesterdayBills.fold<double>(0, (sum, b) => sum + b.total);
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        SingleChildScrollView(
                          ...
                          child: Row(
                            children: [
                              _buildMicroCard(
                                  'Today\'s Sales',
                                  smallCurrency.format(todayRevenue),
                                  AppTheme.primaryBlue,
                                  Icons.currency_rupee),
                              const SizedBox(width: 12),
                              _buildMicroCard(
                                  'Bills Today',
                                  todayBills.length.toString(),
                                  AppTheme.hotPink,
                                  Icons.receipt),
                              const SizedBox(width: 12),
                              _buildMicroCard(
                        ...
                        if (lowStockItems.isNotEmpty ||
                          ...
                          if (lowStockItems.isNotEmpty &&
                              outOfStockItems
                                  .isEmpty) // Only show this if OOS is empty or separate? Let's show filtered
                            _buildAlertCard(
  ...
  Widget _buildMicroCard(String label, String value, Color color, IconData icon,
      {bool isAlert = false}) {
    return Container(
        ...
        children: [
          ...
          Text(value,
              style: TextStyle(
                  fontSize: 20,
                  fontWeight: FontWeight.bold,
                  color: isAlert ? Colors.red : AppTheme.darkNavy)),
  ...
  Widget _buildSummaryCard(String title, String subtitle, IconData icon,
      Color color, String value, VoidCallback onTap) {
    return GestureDetector(
      onTap: onTap,
      child: Container(
        padding: const EdgeInsets.all(16),
